...
public interface ChannelRepository extends Neo4jRepository<Channel, Long> {
        ...
        @Query("MATCH (s:Server) WHERE id(s) = $serverId "
                        ...
                        + "RETURN c")
        Channel createInServer(@Param("serverId") Long serverId, @Param("name") String name,
        ...
        @Query("MATCH (c:Channel) WHERE id(c) = $channelId "
                        ...
                        + "OPTIONAL MATCH (m)-[rb:POSTED_BY]->(u:User) "
                        + "WITH m, rb, u "
                        + "OPTIONAL MATCH (m)-[r:CONTAINS]->(w:Word) "
                        + "RETURN m, rb, u, collect(r), collect(w)")
        List<Message> findMessagesWithWords(@Param("channelId") Long channelId);

        @Query("MATCH (c:Channel) RETURN id(c)")
